...
const StyledButton = styled(Button)(({ theme, buttonFunc, powerPermission }) => ({
  marginRight: '10px',
  backgroundColor: buttonFunc === "Phân quyền" ? (powerPermission
    ? theme.colors.greenAccent[600]
    : theme.colors.redAccent[600]) : theme.colors.greenAccent[600],
  width: '130px',
  '& .MuiButton-label': {
    color: 'white',
  },
...
const ButtonCell = ({ buttonFunc, powerPermission, onClick, className }) => {
  const endIcon = buttonFunc === "Chi tiết"
    ? <DescriptionIcon />
    : <PersonAddIcon />;
